...
- `app/_layout.tsx` `checkAuth`: cache-first cold-start (2026-05-09 revision). Returning users with `READY_CACHE_KEY` set transition to `ready` after a single AsyncStorage read (~10 ms) and `verifyAuthInBackground()` re-validates the auth session + `banned_at` in parallel with tabs mounting. Banned users are kicked to the banned screen within ~3-5 s of cold start; in that window migration 26 RLS gates (`is_banned()` on messages/notifications/reports/buddy_sessions) prevent them from doing anything. First-time / signed-out users fall through to the original sequential flow with the 20 s timeout, so no regression for new accounts. Trade-off vs. the old "synchronous banned_at check on every launch" decision: cold-start UX > immediate banned response, with RLS as defense in depth.
